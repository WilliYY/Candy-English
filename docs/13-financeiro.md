...
**Validacao inicial em 10/09/2026:** codigo `9d4e603`, schema/typecheck/lint, 266 testes unitarios e build Docker aprovados. Smoke HTTP na candidata confirmou mensalidade, cancelamento, autor, permissao, duplo clique e ausencia de valores de terceiros no HTML/RSC. Doces revelaram a constraint antiga `Sale_settlement_check` (`P2039`/`23514`), que proibia toda fatura mensal paga. O responsavel autorizou a correcao pontual. A migration `20260910150000_allow_monthly_invoice_settlement` permite `paidAt` em faturas mensais, preserva as demais validacoes e nao atualiza registros. Exige backup e validacao candidata antes de recriar o app; a evidencia de publicacao sera registrada apos os smokes. Fixtures iniciais removidas, sem alterar cobrancas reais. Auditoria npm: zero criticas, 12 altas preexistentes (fora do escopo). Conferencia visual inicial interrompida por timeout do navegador; nao aceita como validada.
