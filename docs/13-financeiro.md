...
**Estado da entrega em 10/09/2026: codigo `9d4e603` enviado, ainda nao publicado.** Schema/typecheck/lint e 266 testes unitarios passaram; build Docker aprovado. Smoke HTTP na candidata confirmou mensalidade, cancelamento, autor, permissao, duplo clique e ausencia de valores de terceiros no HTML/RSC. Pagamento de doces falhou e fez rollback: Prisma `P2039`, PostgreSQL `23514`, constraint `Sale_settlement_check` da migration `20260823233000_add_sales_pos` exige `paidAt IS NULL` para toda venda `MONTHLY_INVOICE`. E necessario permitir `paidAt` nas faturas mensais sem retirar as demais validacoes. Migração nao criada/aplicada: aguarda confirmacao do responsavel antes de alterar o banco de producao. Fixtures removidas; nao houve alteracao de cobrancas reais. Auditoria npm: zero criticas, 12 altas preexistentes (fora do escopo). Conferencia visual interrompida por timeout do navegador; nao aceita como validada.
...
- Implementacao: `src/app/ava/teacher/finance-actions.ts`, `src/lib/teacher-finance-payment-operations.ts`, `src/lib/teacher-finance-payment.ts`, `src/lib/validations/teacher-finance.ts`, `TeacherPaymentButton` e `TeacherProductPayments`. Sem migration nem dependencia nova.
