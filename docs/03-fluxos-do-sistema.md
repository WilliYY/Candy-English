...
16. O painel mostra estados de `Incompleto`, `Pronto`, `Convertendo`, `Convertido` e `Erro`; o botao `Criar aluno no AVA` fica bloqueado ate email/login, senha e confirmacao final estarem validos.
17. A action protegida valida email/login e senha no servidor, bloqueia duplicidade de email/login em `User` e em outro pre-cadastro, hash a senha com `bcryptjs` e converte em uma transaction: cria `User.role=STUDENT`, `StudentProfile`, `StudentTeacherAssignment` quando houver teacher, `FinancialStudent`, snapshots em `FinancialPayment`, `AgendaStudent`, ocorrencias futuras em `AgendaLesson`, logs simples e grava `convertedUserId`, `convertedStudentProfileId`, `convertedFinancialStudentId` e `convertedAgendaStudentId` no pre-cadastro. Se algo falhar, nada fica criado pela metade.
18. `APPROVED` continua sendo o status tecnico exibido como `Convertido`. O campo minimizado `Contexto Catty`, quando preenchido, grava uma memoria pessoal inicial segura do aluno em `CattyUserMemory` depois da conversao.
19. Teacher nao recebe lista de financeiro, gastos, agenda completa ou pre-cadastros de outras teachers; a conversao dela cria apenas os registros linkados daquele interessado.
