...
19. Se faltar mensalidade, dia de pagamento ou forma, a UI e a action bloqueiam a conversao antes de criar qualquer dado. Se faltarem dias ou horario de agenda, Admin/Teacher podem confirmar que vao preencher depois; nesse caso a transaction cria `AgendaStudent` linkado sem ocorrencias em `AgendaLesson` e registra essa pendencia no log.
