...
- A leitura automatica do texto do `LISTENING` usa a rota protegida `/ava/homework-listening-detect`; no editor, o client tenta primeiro ler a camada de texto do PDF localmente quando existir, depois envia um recorte maior e com contraste da area renderizada para reduzir atraso e melhorar precisao, mantendo fallback para o PDF/imagem protegido com coordenadas percentuais quando o recorte nao estiver disponivel ou nao trouxer texto. A rota retorna apenas o texto detectado; `STUDENT` nao acessa essa rota.
