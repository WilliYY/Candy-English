...
- O acesso ao arquivo passa por rota protegida; student so abre arquivo da propria homework. A resposta usa cache privado `no-store`; o visualizador interno recebe o PDF inline, enquanto `?download=1` devolve `Content-Disposition: attachment` para a acao secundaria baixar o original sem abrir o visualizador externo do navegador.
...
- A tela `Criar/Ver Homework` mantem a criacao em blocos visuais separados para selecao/dados, instrucoes/upload e fila de criacao; o seletor de alunos fica minimizado por padrao com resumo dos selecionados e opcao de abrir a lista ou marcar todos. Quando todos estao marcados, o resumo mostra uma frase curta com o total, sem comprimir o checkbox nem empilhar todos os nomes, e oferece `Limpar` como acao separada. O titulo do grupo e o checkbox `Selecionar todos` usam rotulos acessiveis distintos; a lista mostra atividade, aluno, aula, arquivo, status de salvamento e quantidade de areas em cards mais escaneaveis antes de abrir o editor, com cores distintas para item, aula, arquivo, replicacao e pronto/salvo.
- A tela `Aulas` mantem a mesma base tecnica, mas mostra resumo proprio de aulas, areas e alunos, formulario organizado em participantes, detalhes e material, acentos visuais por etapa e cards de `Aula interativa` separados visualmente dos cards de homework. O formulario usa cabecalho de alto contraste, etapa azul para participantes, violeta para detalhes, amarela para resumo e verde para material; campos e acao principal permanecem legiveis no desktop e empilham sem rolagem horizontal no mobile.
...
- No mobile, o comando `Abrir atividade`/`Abrir` deve permanecer visivel no resumo. A homework interativa mais recente abre automaticamente mesmo quando existem varias, e a aula mais recente tambem abre com sua primeira atividade para expor o conteudo novo sem uma etapa escondida. Os itens antigos continuam recolhidos para nao carregar varios PDFs ao mesmo tempo. Em desktop, o comportamento operacional recolhido e preservado, salvo item unico ja configurado para abrir por padrao.
- No mobile abaixo de `640px`, o PDF/imagem do aluno usa uma largura minima de leitura dentro de uma regiao com rolagem horizontal, preservando as coordenadas percentuais dos campos. PDFs renderizam paginas somente quando se aproximam da viewport e usam densidade de canvas reduzida no celular para evitar tela vazia por excesso de memoria. Imagem, PDF e pagina possuem estado de carregamento/erro visivel e acao protegida `Baixar arquivo original` como contingencia, sem navegar para uma interface externa; tablet e desktop mantem o ajuste responsivo normal.
...
- Autosave, entrega e reabertura do mesmo aluno/homework devem usar a mesma chave de lock transacional; um autosave atrasado nunca pode rebaixar `SUBMITTED` ou `REVIEWED` para `DRAFT`, e apenas a primeira entrega concorrente pode mudar `DRAFT`/`RETURNED` para `SUBMITTED`.
- No aluno, o autosave de `DRAFT` grava em segundo plano sem revalidar a pagina inteira, para nao fechar o card aberto nem sobrescrever texto digitado com dados antigos. A tela tambem mantem uma copia local temporaria do rascunho no navegador enquanto a atividade nao foi entregue/corrigida, como protecao contra falha de rede ou refresh acidental. Essa copia expira 7 dias apos a ultima alteracao e e removida ao entregar, bloquear a atividade ou sair do AVA. Se uma aba antiga chamar uma Server Action de outro deploy ou a conexao falhar, o erro tecnico e capturado no card, o rascunho local recente permanece e o aluno recebe a acao `Atualizar atividade`.
- `SUBMITTED` e entrega oficial e gera evento para teacher/admin; uma segunda chamada de entrega nao pode sobrescrever as respostas ja entregues.
