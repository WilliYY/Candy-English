...
- `OPENAI_API_KEY` e opcional; quando existe, ativa o modo OpenAI da Catty apenas quando usuario autorizado chama Catty pelo nome, pode ser usada pelo OCR opcional de homework, pela leitura automatica do texto dos campos Listening e pelo audio dos campos Listening de homework/aula interativa. Sem a chave, Catty cai para Gemini/fallback e o texto do Listening deve ser preenchido manualmente.
- `OPENAI_CATTY_MODEL` define o modelo OpenAI da Catty, com fallback para `gpt-5.4-nano`.
- `OPENAI_HOMEWORK_OCR_MODEL` define o modelo OpenAI usado pelo OCR opcional de homework e pela leitura do texto dentro do box Listening, com fallback para `gpt-4.1-mini`.
