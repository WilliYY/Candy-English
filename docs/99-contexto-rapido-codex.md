...
- O financeiro admin em `/ava/admin?task=financeiro` e uma area propria com blocos internos `Alunos` e `Pagamentos`: `Alunos` mostra linhas mensais tipo planilha com nome, polo, mensalidade, vencimento, situacao e `Pago hoje`, seletor de mes com setas, totais e filtros; o painel lateral preserva historico e edicoes. `Pagamentos` registra gastos internos em `FinancialExpense`, por mes e unidade, separado dos alunos. `/ava/teacher?task=financeiro` seleciona todos os alunos ativos e projeta nome/polo/status/vencimento/data confirmada, sem valor, forma, contato, gasto, venda, log ou exportacao.
- O PDV em `/ava/vendas` permite Admin/Teacher pesquisar e manter produtos, controlar estoque, registrar venda paga na hora ou cobrar na competencia mensal de um aluno cadastrado. Teacher seleciona qualquer aluno ativo e ve vendas proprias. Compras mensais ficam em `Sale`, separadas dos snapshots de `FinancialPayment`.
...
- A agenda admin em `/ava/admin?task=agenda` e uma agenda interna simples, separada dos alunos do AVA: usa `AgendaStudent` para o aluno interno, `AgendaLesson` para ocorrencias reais e `AgendaLog` para historico. A tela abre no mes atual/dia de hoje e prioriza uma planilha mensal por aluno, agrupada por `Polo 1 - Ivate` e `Polo 2 - Douradina`, com busca, filtros de situacao e colunas de rotina, aulas, presencas, faltas, pendencias e proxima aula. O dia selecionado preserva os controles `Veio`/`Nao veio`/`Resetar`; a ficha do aluno mantem edicao da rotina, presencas/faltas e historico. `AgendaStudent.isActive`, `defaultTime` e `weekdayMask` guardam a rotina atual; inativar ou editar desativa ocorrencias futuras sem apagar historico antigo nem criar `User`/`StudentProfile`.
