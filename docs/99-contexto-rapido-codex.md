...
- O financeiro admin em `/ava/admin?task=financeiro` e uma lista simples de cards por aluno pagante, com total previsto/recebido/pendente/atrasado no topo, busca por nome/telefone, filtro por status, acao rapida `Pago hoje`, detalhe de historico ao clicar no aluno, edicao mensal de valor/data/observacao, edicao de dados fixos a partir do mes selecionado e parcelas opcionais salvas como snapshots em `FinancialPayment`; registros antigos sem parcela continuam como mensalidade recorrente.
