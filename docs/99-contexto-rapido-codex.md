...
- `/ava/secretaria` usa `SECRETARIA_PERMISSION_MATRIX` em `src/lib/roles.ts`: Admin ve Cadastro, agenda, contratos, administracao e APIs; Teacher cadastra alunos para a propria teacher e ve somente registros antigos proprios/atribuidos e contratos permitidos; Student nao acessa. Financeiro completo fica exclusivo de Admin, enquanto Teacher recebe apenas situacao mensal sem valores dos alunos vinculados; Vendas fica para Admin/Teacher; Ponto fica para Admin e Teacher individualmente habilitado.
- Contratos: Admin pode excluir qualquer contrato; Teacher somente contrato individual de aluno vinculado, nunca contrato geral; Student nao exclui. A server action revalida role e vinculo antes de remover o registro e o PDF.
