...
- `/ava/secretaria` usa `SECRETARIA_PERMISSION_MATRIX` em `src/lib/roles.ts`: Admin ve pre-cadastros, financeiro, agenda, gastos/unidades, relatorios simples, contratos, administracao e APIs; Teacher ve somente pre-cadastros proprios/atribuidos e contratos permitidos; Student nao acessa.
...
- `/ava/escolha` deve permanecer limpa, sem sidebar de trabalho. A sidebar completa fica em `AvaWorkspaceShell` e so envolve `/ava/admin`, `/ava/teacher`, `/ava/student` e `/ava/secretaria`, recebendo o modo `AVA`, `SECRETARIA` ou `STUDENT` para nao misturar menus.
...
- O financeiro admin em `/ava/admin?task=financeiro` tem blocos internos `Alunos` e `Pagamentos`: `Alunos` mostra cards por aluno pagante, unidades `Unidade 1 Ivaté`/`Unidade 2 Douradina`, total previsto/recebido/pendente/atrasado, busca por nome/telefone, filtro por status/unidade, acao rapida `Pago hoje`, historico ao clicar no aluno, edicao mensal de valor/data/observacao, edicao de dados fixos a partir do mes selecionado e parcelas opcionais salvas como snapshots em `FinancialPayment`; `Pagamentos` registra gastos internos da loja em `FinancialExpense`, por mes e unidade, separado dos alunos.
