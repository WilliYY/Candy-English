...
- O financeiro admin em `/ava/admin?task=financeiro` e uma area propria com blocos internos `Alunos` e `Pagamentos`: `Alunos` mostra linhas mensais tipo planilha com nome, polo, mensalidade, vencimento, situacao e `Pago hoje`, seletor de mes com setas, totais e filtros; tambem mostra as faturas pessoais de doces da equipe, discriminadas de mensalidade, e permite ao Admin confirmar/reabrir o recebimento. `Pagamentos` registra gastos internos em `FinancialExpense`, por mes e unidade. `/ava/teacher?task=financeiro` projeta todos os alunos ativos sem valores e, em bloco isolado, itens/total somente da fatura pessoal da Teacher autenticada.
- O PDV em `/ava/vendas` permite Admin/Teacher pesquisar e manter produtos, controlar estoque, registrar venda paga na hora ou cobrar na competencia mensal de um aluno ou professor cadastrado. Professor aparece identificado no seletor; a compra marcada fica em `Sale.buyerUserId`, o perfil mostra `Fatura pendente` e o clique abre o Financeiro. Compras de alunos continuam ligadas a `FinancialPayment`; conta de equipe nunca se mistura com mensalidade.
