...
- Catty so responde para usuario logado no AVA; `/api/catty/chat` exige `auth()` com role `ADMIN`, `TEACHER` ou `STUDENT`, detecta uma intencao leve da mensagem antes de chamar IA/fallback, incluindo correcao, traducao, explicacao, homework, motivacao, pergunta confusa ou pergunta grande misturada, usa historico persistido/local recente limitado para contexto, usa Gemini quando configurado, OpenAI apenas quando a mensagem chama Catty pelo nome, fallback local autorizado quando nao ha chave, a chamada falha ou a resposta da IA vem vazia/cortada/generica, e mantem personalidade oficial em `CATTY_PERSONALITY_GUIDE`.
