...
- A agenda admin em `/ava/admin?task=agenda` e uma agenda interna simples, separada dos alunos do AVA: usa `AgendaStudent` para o aluno interno, `AgendaLesson` para ocorrencias reais e `AgendaLog` para historico. A tela abre no mes atual/dia de hoje, mostra calendario mensal com contagem por dia, lista do dia selecionado, busca por nome/telefone, cadastro recorrente por dias da semana e horario, cards com `Veio`/`Nao veio`/`Resetar`, detalhe do aluno com edicao da rotina, presencas/faltas e historico. `AgendaStudent.isActive`, `defaultTime` e `weekdayMask` guardam a rotina atual; inativar ou editar desativa ocorrencias futuras sem apagar historico antigo nem criar `User`/`StudentProfile`.
