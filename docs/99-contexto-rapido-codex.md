...
- `/ava/escolha` deve permanecer limpa, sem sidebar de trabalho. Admin ve `AVA`, `SECRETARIA`, `FINANCEIRO` e `VENDAS`; Teacher ve `AVA`, `SECRETARIA` e `VENDAS`; Student entra direto. `AvaWorkspaceShell` recebe `AVA`, `SECRETARIA`, `FINANCEIRO`, `VENDAS` ou `STUDENT` para nao misturar menus.
