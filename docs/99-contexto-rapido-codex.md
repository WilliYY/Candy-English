...
- Catty tambem tem detector local de correcao conversacional para erros comuns de ingles (`I likes`, `She like`, idade com `have`, `went in`, `yesterday` com presente, plural, artigo `a/an`, `do/does`, `was/were`, `is/are`): quando a frase ja foi enviada, ela responde com reacao curta, `Melhor: ...`, explicacao simples e pergunta relacionada; em homework, usa como estrutura parecida sem entregar gabarito.
