...
- Catty User Artifacts fica em `CattyUserArtifact` e na tarefa `Estilo da Catty`: Admin gerencia todos, Teacher alunos vinculados e Student apenas sugere/desativa o proprio tema. Campos principais: `themeId`, `label`, `emojis`, `catchphrases`, `sounds`, `example`, `toneRule`, `status`, `lastUsedAt` e `usageCount`. Somente `ACTIVE` entra no prompt/fallback e nos baloes locais do AVA logado daquele usuario; `PENDING`, `DISABLED` e `ARCHIVED` ficam fora. Ativar sincroniza memoria `FAVORITE_THEME/artifact_*`; desativar sincroniza `STYLE/avoid_*`. Admin/Teacher tambem podem pedir enriquecimento de tema, que usa `CattyArtifactEnrichmentCache`/`CattyArtifactEnrichment` para criar sugestoes cacheadas e revisaveis; busca web e opcional por `CATTY_ARTIFACT_SEARCH_PROVIDER=brave` + `BRAVE_SEARCH_API_KEY`, nunca roda no chat normal, e so ativa depois de aprovacao/edicao humana.
