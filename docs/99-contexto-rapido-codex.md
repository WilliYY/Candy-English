...
- Homeworks interativas reais criadas em `Criar/Ver Homework` podem ser compartilhadas com alunos extras por `HomeworkStudentAssignment`, sem duplicar PDF/campos/aula interna; cada aluno continua com sua propria `HomeworkSubmission`. Teacher so adiciona alunos vinculados e Admin adiciona alunos ativos; aulas interativas `lesson-manual` nao usam esse compartilhamento extra.
