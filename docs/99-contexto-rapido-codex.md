...
- `/ava/secretaria` usa `SECRETARIA_PERMISSION_MATRIX` em `src/lib/roles.ts`: Admin ve pre-cadastros, agenda, contratos, administracao e APIs; Teacher ve somente pre-cadastros proprios/atribuidos e contratos permitidos; Student nao acessa. Financeiro fica exclusivo de Admin; Vendas fica para Admin/Teacher; Ponto fica para Admin e Teacher individualmente habilitado.
- Pre-cadastros sao criados manualmente dentro da Secretaria depois do contato pelo WhatsApp; Admin ve todos, Teacher ve apenas criados por ela ou atribuidos a ela, e a busca do painel roda no client apenas sobre essa lista autorizada, normalizando acentos e telefone/documento por digitos. O registro salva unidade, teacher responsavel, agenda pretendida e combinado de pagamento. No clique em `Tornar aluno`, a UI abre um painel/modal em etapas com dados do aluno, login do AVA, financeiro, agenda e confirmacao; exige email/login revisado e senha inicial visivel/editavel gerada por nome simplificado + `candy`; uma transaction cria User STUDENT com hash seguro, StudentProfile, vinculo teacher quando aplicavel, financeiro e agenda, salvando os ids linkados no pre-cadastro.
- `/ava/escolha` deve permanecer limpa, sem sidebar de trabalho. Admin ve `AVA`, `SECRETARIA`, `FINANCEIRO`, `VENDAS` e `PONTO`; Teacher ve `AVA`, `SECRETARIA`, `VENDAS` e `PONTO` somente com perfil ativo; Student entra direto. `AvaWorkspaceShell` recebe tambem `PONTO` sem misturar menus.
- Admin gerencia usuarios, cria/acompanha/converte pre-cadastros e usa `Alunos cadastrados` na Secretaria para localizar tanto alunos criados diretamente quanto convertidos, editar nome/email/telefone/polo e conferir ligacoes com teacher, financeiro e agenda. A action segue exclusiva de `ADMIN`; Teacher ve apenas alunos vinculados e em modo de consulta.
- O financeiro admin em `/ava/admin?task=financeiro` e uma area propria com blocos internos `Alunos` e `Pagamentos`: `Alunos` mostra linhas mensais tipo planilha com nome, polo, mensalidade, vencimento, situacao e `Pago hoje`, seletor de mes com setas, totais e filtros; o painel lateral preserva historico e edicoes. `Pagamentos` registra gastos internos em `FinancialExpense`, por mes e unidade, separado dos alunos.
...
- Teacher cria aulas, homework interativo, materiais, feedback, mensagens, contratos, ve a aula ao vivo em manutencao temporaria e pode criar/acompanhar/converter seus pre-cadastros como alunos `STUDENT` vinculados a sua teacher, com financeiro/agenda criados apenas para aquele interessado na transaction de conversao.
