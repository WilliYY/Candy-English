...
- Logos principais usam `BrandLogo` com `public/brand/candy-logo-animated.webm` em loop transparente e poster `public/brand/candy-logo-animated-poster.png`; manter `object-contain` e fallback para `prefers-reduced-motion`. No header da home, a logo fica em janela arredondada com `overflow-hidden` para a animacao nao escapar da barra branca.
