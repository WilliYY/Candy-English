...
- O PDV em `/ava/vendas` permite Admin/Teacher pesquisar e manter produtos, controlar estoque, registrar venda paga na hora ou cobrar na competencia mensal de qualquer aluno ativo ou professor cadastrado. Produto zerado fica vermelho e nao entra no carrinho. Para aluno, a compra usa `FinancialPayment` somente se a mensalidade estiver aberta; caso contrario vira fatura apenas de produtos, sem reabrir o pagamento escolar. Professor aparece identificado no seletor; a compra marcada fica em `Sale.buyerUserId`, o perfil mostra `Fatura pendente` e o clique abre o Financeiro.
