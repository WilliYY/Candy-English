...
Publicação da Rotina confirmada em 13/09/2026: `b6e15d3`/`bc9d03e`, 313 testes,
typecheck/lint/build e 46 checks publicados de servidor/auth/avatar aprovados.
Skeleton só após autorização (Suspense interno); anônimo recebe HTTP 307,
callback/data preservados. App saudável, canal pausado e fixtures removidas.
...
Publicacao 12/09/2026: Catty WhatsApp em `/ava/whatsapp`, somente ADMIN, instalada e pausada, sem numero pareado, contatos ou mensagens. Evolution/worker/banco exclusivos; nunca compartilhar sessao/chaves nem reiniciar Miauby. `COMPOSE_FILE` no `.env` inclui `docker-compose.whatsapp.yml`; preservar esse overlay em futuros deploys. Migration `20260911140000_catty_whatsapp`, backup/restore, 285 testes e smokes candidate/published aprovados. Nenhum comando financeiro/administrativo por WhatsApp. Pareamento e conferencia visual autenticada pendentes. Operacao, limites, seguranca e evidencias em `docs/24-catty-whatsapp.md`.
...
- `/ava/secretaria` usa `SECRETARIA_PERMISSION_MATRIX` em `src/lib/roles.ts`: Admin ve Cadastro, agenda, contratos, administracao e APIs; Teacher cadastra alunos para a propria teacher e ve somente registros antigos proprios/atribuidos; Student nao acessa. Financeiro completo fica exclusivo de Admin, enquanto Teacher recebe apenas situacao mensal sem valores de todos os alunos ativos; Vendas fica para Admin/Teacher; Ponto fica para Admin e Teacher individualmente habilitado.
- Contratos: Admin pode excluir qualquer contrato; Teacher somente contrato individual, nunca contrato geral; Student nao exclui. A server action revalida role e contrato antes de remover o registro e o PDF.
- O fluxo novo `Cadastro` exige email/login, senha inicial editavel e confirmacao e cria, em uma unica transaction, `StudentPreRegistration` auditavel ja convertido, User STUDENT com hash seguro, StudentProfile, vinculo teacher quando aplicavel, financeiro e agenda. Registros antigos abertos continuam em `Cadastros anteriores` com busca, edicao e `Tornar aluno`; nao sao apagados nem recriados.
- `/ava/escolha` deve permanecer limpa, sem sidebar de trabalho. Admin ve `AVA`, `SECRETARIA`, `FINANCEIRO`, `VENDAS` e `PONTO`; Teacher ve `AVA`, `SECRETARIA`, `FINANCEIRO` limitado, `VENDAS` e `PONTO` somente com perfil ativo; Student entra direto. `AvaWorkspaceShell` separa os modos sem misturar menus.
- Admin gerencia usuarios, cadastra alunos com AVA liberado no mesmo envio e usa `Alunos cadastrados` na Secretaria para localizar todos, editar nome/email/telefone/polo e conferir ligacoes com teacher, financeiro e agenda. A edicao cadastral segue exclusiva de `ADMIN`; Teacher ve todos os alunos ativos em modo de consulta e pode atuar nos fluxos pedagogicos permitidos.
- Na base Admin, `Excluir conta` exige motivo + texto `EXCLUIR`, encerra sessoes, remove a conta das listagens e agenda anonimização em 2 anos. A rotina `retention:anonymize-users -- --apply` limpa identidade, senha, contato, avatar e perfil pessoal, mas preserva uma linha tecnica para nao quebrar historicos relacionados.
- O financeiro admin em `/ava/admin?task=financeiro` e uma area propria com blocos internos `Alunos` e `Pagamentos`: `Alunos` mostra linhas mensais tipo planilha com nome, polo, mensalidade, vencimento, situacao e `Pago hoje`, seletor de mes com setas, totais e filtros; tambem mostra faturas separadas de produtos de alunos e professores, discriminadas da mensalidade, e permite ao Admin confirmar/reabrir o recebimento. `Pagamentos` registra gastos internos em `FinancialExpense`, por mes e unidade. `/ava/teacher?task=financeiro` projeta todos os alunos ativos sem valores e, em bloco isolado, itens/total somente da fatura pessoal da Teacher autenticada.
- O PDV em `/ava/vendas` permite Admin/Teacher pesquisar e manter produtos, controlar estoque, registrar venda paga na hora ou cobrar na competencia mensal de qualquer aluno ativo ou professor cadastrado. Produto zerado fica vermelho e nao entra no carrinho. Para aluno, a compra usa a mensalidade aberta do mes atual; se estiver paga/inativa, usa a mensalidade aberta do proximo mes. Se a proxima nao estiver aberta, bloqueia sem consumir estoque. Somente a ausencia de mensalidade no mes atual mantem a fatura apenas de produtos. Mensalidade e doces aparecem discriminados por nome, quantidade e valores; nenhum pagamento escolar e reaberto. Professor aparece identificado no seletor; a compra marcada fica em `Sale.buyerUserId`, o perfil mostra `Fatura pendente` e o clique abre o Financeiro.
- O ponto em `/ava/ponto` aceita batidas ilimitadas alternadas, justificativa opcional, horario do servidor e anti-duplicacao. Admin habilita pessoas, adiciona/corrige batidas com revisao imutavel e baixa PDF; Teacher ativo ve apenas o proprio espelho.
- A agenda admin em `/ava/admin?task=agenda` usa `AgendaStudent` vinculado 1:1 ao `StudentProfile`, `AgendaLesson` para ocorrencias reais e `AgendaLog` para historico. A tela abre no mes atual/dia de hoje e prioriza uma planilha mensal por aluno, agrupada por `Polo 1 - Ivate` e `Polo 2 - Douradina`, com busca, filtros de situacao e colunas de rotina, aulas, presencas, faltas, pendencias e proxima aula. O dia selecionado preserva os controles `Veio`/`Nao veio`/`Resetar`; a ficha do aluno mantem edicao da rotina, presencas/faltas e historico. Todo aluno criado pela Secretaria, Admin, Financeiro, Agenda ou mobile Admin garante Financeiro e Agenda vinculados na mesma transaction; dados incompletos ficam como `Completar`, sem criar outro aluno por nome.
- Teacher cria aulas, homework interativo, materiais, feedback, mensagens, contratos, ve a aula ao vivo em manutencao temporaria e pode cadastrar alunos `STUDENT` vinculados a propria teacher, com financeiro/agenda criados apenas para aquele aluno na transaction; registros antigos proprios/atribuidos continuam convertiveis.
- Student acessa aulas, homework, mensagens, contratos, perfil, avatar e ve a aula ao vivo em manutencao temporaria. Aluno sem foto ve um popup diario limpo com o video `public/brand/profile-photo-popup.mp4`; clicar no video abre `/ava/student?task=perfil`, e fechar marca o dia visto no navegador.
...
- Candy XP usa `src/lib/candy-xp.ts` para curva infinita e `src/lib/candy-xp-persistence.ts` para eventos `CandyXpEvent`, perfil/cache `CandyXpProfile`, badges e catalogo inicial de missoes com `sourceKey` anti-duplicacao. A primeira foto do perfil do student vale 500 XP na fonte `Foto do perfil` e usa `sourceKey` fixa para nao duplicar em reenvio/troca.
...
- Homeworks interativas reais criadas em `Criar/Ver Homework` podem ser replicadas para outro aluno pelo card do editor. A action cria uma nova `Lesson` e um novo `Homework` para o aluno escolhido, copia perguntas e `HomeworkInteractiveField` (`Texto`, `Letra/Num`, `Marcar`, `Desenho`, `Listening`) e aponta `replicatedFromHomeworkId` para a origem. O PDF/imagem otimizado pode ser reaproveitado por caminho de storage, mas cada aluno ve um item proprio e tem `HomeworkSubmission`, rascunho, entrega e correcao isolados. Admin e Teacher replicam para qualquer aluno ativo; aulas interativas `lesson-manual` nao usam esse fluxo.
...
- Atividades criadas por `Aulas` aparecem em `Aulas e Materiais`.
- Atividades criadas por `Criar/Ver Homework` aparecem em `Responder homework`.
- No Student mobile, homework e atividade interativa mostram acao clara de abrir/fechar; a homework mais recente abre automaticamente mesmo quando ha varias, e a aula mais recente abre com sua primeira atividade. O PDF/imagem usa modo de leitura com pagina minima de `560px` abaixo de `640px`, rolagem horizontal local, renderizacao gradual de paginas, menor densidade de canvas e fallback protegido para baixar o arquivo original quando houver falha visual, sem abrir visualizador externo. Itens antigos ficam recolhidos para nao carregar varios PDFs, tablet/desktop continuam responsivos sem largura minima e falha de Server Action/conexao fica contida no card com rascunho local preservado e acao de atualizar.
...
- No hero da home, o conteudo de intro e um banner/carrossel em `src/components/site/home-banner-carousel.tsx` com `intro-1.mp4`, `home-banner-aula-julho-2026.jpeg` como segundo slide e `intro-2.mp4`, setas, bolinhas, troca automatica em 8 segundos, videos iniciados por clique e pausa da troca automatica quando um video e reproduzido.
- Logos principais usam `BrandLogo` com `public/brand/candy-logo-animated.webm` em loop transparente e poster `public/brand/candy-logo-animated-poster.png`; manter `object-contain` e fallback para `prefers-reduced-motion`. No header da home, a logo fica em janela arredondada com `overflow-hidden`, escala interna menor e encaixe levemente mais baixo para a animacao nao escapar nem cortar na barra branca. No topo interno do AVA, manter a logo animada em janela arredondada mais alta, com `overflow-hidden`, escala controlada e hover leve para aparecer inteira sem passar da sidebar. O login usa uma unica logo estatica no topo (`BrandLogo animated={false}`); o video de fundo e os baloes decorativos podem ser pausados e respeitam movimento reduzido.
- O componente `src/components/site/home-hero-loop-video.tsx` garante autoplay mudo do video principal; `src/components/site/home-banner-carousel.tsx` controla slides, setas, indicadores, play/pause e som do banner.
...
- Vendas/PDV: `docs/19-vendas.md`
- Ponto: `docs/22-ponto.md`
...
- Desde 2026-09-08, por pedido do responsavel, login web/mobile usa apenas email e senha para todas as roles; codigo, exigencia TOTP e painel/actions MFA foram retirados. `UserMfa` antigo fica cifrado/inativo funcionalmente, sem consulta no login nem migration destrutiva. `mfaCode` opcional e aceito apenas por compatibilidade mobile. A tela de login usa `login.module.css` e `LoginExperience`, com campos grandes, Hello! editorial, baloes suaves, video pausavel e respeito a movimento reduzido.
- A matriz dos 15 controles fica em `docs/11-seguranca.md`; CI/Dependabot e monitor de health, containers, disco, backup e falhas de login estao no repositorio, enquanto DDoS/WAF, copia externa e pentest dependem de operacao externa.
...
- `TEACHER` atua apenas sobre aulas/homeworks proprios, mas pode selecionar qualquer aluno ativo como destinatario; `StudentTeacherAssignment` e referencia opcional, nao liberacao.
