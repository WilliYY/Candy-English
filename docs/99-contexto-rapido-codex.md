...
- Catty User Artifacts fica em `CattyUserArtifact` e na tarefa `Estilo da Catty`: Admin gerencia todos, Teacher alunos vinculados e Student apenas sugere/desativa o proprio tema. Campos principais: `themeId`, `label`, `emojis`, `catchphrases`, `sounds`, `example`, `toneRule`, `status`, `lastUsedAt` e `usageCount`. Somente `ACTIVE` entra no prompt/fallback; `PENDING`, `DISABLED` e `ARCHIVED` ficam fora. Ativar sincroniza memoria `FAVORITE_THEME/artifact_*`; desativar sincroniza `STYLE/avoid_*`.
