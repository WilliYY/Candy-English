...
- Catty tem banco de exemplos em `src/lib/catty-examples.ts`, repertorio ampliado em `src/lib/catty-scenarios.ts`, guia em `docs/catty-comportamento.md` e smoke `npm run audit:catty-behavior` para validar tom, intencoes, fallback, redirecionamento de codigo/API/off-topic, bloqueio de resposta pronta, no maximo um bordao por resposta, ate dois emojis permitidos, personalizacao segura por nome, memoria global aprovada, memoria pessoal segura, artefatos padrao/customizados por interesse, enriquecimento revisavel/cacheado de artefatos, anti-repeticao por historico recente, 51 cenarios curados, 20 correcoes comuns, 6 conversas continuas e gatilho OpenAI apenas por `Catty`.
- O repertorio `catty-scenarios` tem nome, intencao, entrada do usuario, contexto opcional, memoria opcional, resposta ruim, resposta ideal, regra e tags; a rota seleciona ate 4 cenarios por intencao/contexto/mensagem/memoria e inclui no prompt como `Cenarios de repertorio da Catty`. Quando a IA falha e ha match forte, o fallback do servidor pode usar a resposta ideal curada antes do fallback generico.
- Catty tambem tem detector local de correcao conversacional para erros comuns de ingles (`I likes`, `She like`, idade com `have`, `went in`, `yesterday` com presente, plural, artigo `a/an`, `do/does`, `was/were`, `is/are`): quando a frase ja foi enviada, ela responde com reacao curta, `Melhor: ...`, explicacao simples e pergunta relacionada; em homework, usa como estrutura parecida sem entregar gabarito. A continuidade multi-turno usa historico curto para juntar gostos com `and` e completar fragmentos seguros, como `red cars` -> `I like red cars.`.
