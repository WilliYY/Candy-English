...
- `/ava/secretaria` usa `SECRETARIA_PERMISSION_MATRIX` em `src/lib/roles.ts`: Admin ve pre-cadastros, agenda, contratos, administracao e APIs; Teacher ve somente pre-cadastros proprios/atribuidos e contratos permitidos; Student nao acessa. Financeiro fica em area visual propria e exclusiva de Admin.
...
- `/ava/escolha` deve permanecer limpa, sem sidebar de trabalho. Admin ve os cards `AVA`, `SECRETARIA` e `FINANCEIRO`; Teacher ve apenas AVA/Secretaria e Student entra direto. `AvaWorkspaceShell` recebe `AVA`, `SECRETARIA`, `FINANCEIRO` ou `STUDENT` para nao misturar menus.
