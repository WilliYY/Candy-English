...
- Catty tambem tem detector local de pedido de pergunta (`faz uma pergunta`, `ask me a question`, `simple past`, carros/comida) e correcao conversacional para erros comuns de ingles (`I likes`, `She like`, idade com `have`, `went in`, `yesterday` com presente, plural, artigo `a/an`, `do/does`, `Does you`, `was/were`, `is/are`): quando a frase ja foi enviada, ela responde com reacao curta, `Melhor: ...`, onde esta o erro e pergunta relacionada; em homework, usa como estrutura parecida sem entregar gabarito. A continuidade multi-turno usa historico curto para juntar gostos com `and` e completar fragmentos seguros, como `chocolate` -> `I like chocolate.` e `red cars` -> `I like red cars.`.
