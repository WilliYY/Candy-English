...
- Admin e Teacher devem passar por uma escolha visual simples no fluxo normal de entrada; a tela deve ser limpa, sem sidebar de trabalho, com logo Candy solta, titulo central compacto e cards curtos. Admin ve `AVA`, `SECRETARIA` e `FINANCEIRO`; Teacher ve apenas `AVA` e `SECRETARIA`. Evitar listas internas, chips explicativos e blocos altos nessa tela.
- A tela inicial da `Secretaria` deve parecer um hub operacional vivo para pre-cadastros, agenda, contratos, gestao e sistema, com movimento leve de entrada/hover respeitando `prefers-reduced-motion`; o Financeiro usa area e grupo de sidebar proprios, sem remover links profundos antigos nem a protecao por role no servidor.
