...
- Admin e Teacher devem passar por uma escolha visual simples entre `AVA` e `Secretaria` no fluxo normal de entrada; a tela deve ser limpa, sem sidebar de trabalho, com logo Candy animada solta e sem fundo/card no topo, titulo central compacto, dois blocos grandes, legiveis e responsivos, faixas Candy vivas, acentos diferentes por area, poucos chips de atalhos essenciais e bullets internos curtos que expliquem rapidamente o escopo da role sem poluir. A sidebar deve separar `AVA` pedagogico de `Secretaria` administrativa sem remover links profundos antigos.
- A tela inicial da `Secretaria` deve parecer um hub operacional vivo: hero com role/polo/escopo em chips coloridos, filtro de polo destacado logo abaixo do titulo, cards de modulos com acentos diferentes para pre-cadastros, financeiro, agenda, contratos, gestao e sistema, movimento leve de entrada/hover respeitando `prefers-reduced-motion` e grupo `Secretaria` aberto na sidebar quando a area atual for administrativa; manter os links profundos e a protecao por role no servidor.
...
- O financeiro admin deve ser intuitivo e escaneavel: ao abrir, mostrar dois blocos claros de navegacao interna (`Alunos` e `Pagamentos`) antes dos detalhes; a aba `Pagamentos` deve parecer controle interno independente, com resumo proprio de gastos/insumos e sem totais de alunos, recebidos ou pendentes; separar visao mensal, seletor de mes, filtro/chips de unidade (`Unidade 1 Ivaté` e `Unidade 2 Douradina`), formulario de novo aluno, formulario de insumos/gastos, lista de alunos e log; em `Pagamentos`, destacar gasto do mes, totais por unidade e lista/tabela compacta de compras salvas para leitura rapida; usar quatro cards de metricas com cores semanticas, uma faixa unica de progresso, meses com dimensao estavel e rolagem horizontal no mobile, cards de aluno tonalizados por status e campos com borda/foco visiveis, mantendo o conceito de controle interno sem parecer gateway de pagamento.
