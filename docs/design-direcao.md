...
- paginas informativas com video em loop no fundo e overlay roxo para leitura; o video deve ser lazy/condicional, tocar apenas em telas a partir de tablet quando `prefers-reduced-motion` permitir, pausar fora da viewport e manter fallback estatico para reduzir lag;
- video fullscreen em loop no login do AVA, mantendo overlay roxo para foco no formulario;
- logo Candy em loop no header da home, no topo do AVA, na marca principal do login, em manutencao e no footer por `BrandLogo`, usando WebM transparente, poster PNG para `prefers-reduced-motion` e dimensoes estaveis com `object-contain`; nas paginas internas do site (`/sobre`, `/metodologia`, `/contato`) o header usa o poster estatico em uma janela arredondada com `overflow-hidden` para a marca nao escapar da barra branca; no topo interno do AVA a logo animada usa janela arredondada mais alta com `overflow-hidden`, escala controlada e hover leve para aparecer inteira sem passar do limite da sidebar; no topo do login do AVA a logo deve ficar estatica para evitar duas animacoes simultaneas na primeira tela;
