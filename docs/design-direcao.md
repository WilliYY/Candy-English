...
- Admin e Teacher devem passar por uma escolha visual simples entre `AVA` e `Secretaria` no fluxo normal de entrada; os dois blocos precisam ser grandes, legiveis e responsivos, com faixas Candy vivas, acentos diferentes por area e chips legiveis sem truncar atalhos importantes. A sidebar deve separar `AVA` pedagogico de `Secretaria` administrativa sem remover links profundos antigos.
