...
- A tela de pre-cadastros da Secretaria deve parecer controle interno, nao landing page: formulario manual no topo, filtros por status, cards escaneaveis com unidade, contato, teacher responsavel, agenda pretendida e combinado de pagamento; conversao para `STUDENT` fica em acao separada e abre painel/modal compacto em etapas (`Dados do aluno`, `Login do AVA`, `Financeiro`, `Agenda`, `Confirmacao`), com estados claros, erro perto dos campos e lembrete de que financeiro/agenda so nascem ao confirmar. O formulario usa blocos coloridos por assunto (`Dados`, `Conversa`, `Agenda`, `Pagamento`, `Observacoes`), campos com borda/foco bem visiveis e cards de interessados com status/unidade destacados para leitura rapida.
