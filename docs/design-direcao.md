...
- O financeiro admin deve ser intuitivo e escaneavel: ao abrir, mostrar dois blocos claros de navegacao interna (`Alunos` e `Pagamentos`) antes dos detalhes; a aba `Pagamentos` deve parecer controle interno independente, com resumo proprio de gastos/insumos e sem totais de alunos, recebidos ou pendentes; separar visao mensal, seletor de mes, formulario de novo aluno, formulario de insumos/gastos, lista de alunos e log; usar cores semanticas para pago, pendente e vencido, mantendo o conceito de controle interno sem parecer gateway de pagamento.
