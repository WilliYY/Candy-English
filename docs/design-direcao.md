...
- Admin e Teacher devem passar por uma escolha visual simples entre `AVA` e `Secretaria` no fluxo normal de entrada; a tela deve ser limpa, sem sidebar de trabalho, com logo Candy animada no topo, titulo central compacto, dois blocos grandes, legiveis e responsivos, faixas Candy vivas, acentos diferentes por area, chips legiveis sem truncar atalhos importantes e bullets internos que expliquem rapidamente o escopo da role. A sidebar deve separar `AVA` pedagogico de `Secretaria` administrativa sem remover links profundos antigos.
