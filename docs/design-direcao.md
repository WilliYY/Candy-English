...
- A tela de pre-cadastros da Secretaria deve parecer controle interno, nao landing page: o topo alterna entre `Novo pre-cadastro` e `Cadastros salvos`, sem uma faixa extensa de status. Toda criacao nasce como `Novo`; os demais status ficam no acompanhamento e em um filtro discreto da lista. O formulario usa blocos coloridos por assunto (`Dados`, `Conversa`, `Agenda`, `Pagamento`, `Observacoes`), campos com borda/foco bem visiveis e largura contida. A lista ordena os mais recentes primeiro e usa cards escaneaveis com status, polo, cidade, dia/horario de criacao, WhatsApp, teacher responsavel, agenda pretendida e combinado de pagamento. A conversao para `STUDENT` fica em acao separada e abre painel/modal compacto em etapas (`Dados do aluno`, `Login do AVA`, `Financeiro`, `Agenda`, `Confirmacao`), com estados claros, erro perto dos campos e lembrete de que financeiro/agenda so nascem ao confirmar.
