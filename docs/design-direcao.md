...
- Na planilha de usuarios, nome completo, email, telefone, polo, perfil e sinais devem quebrar linha em vez de usar reticencias; a acao da linha fica fixa a direita durante a rolagem. O detalhe aberto funciona como uma gaveta de gestao limitada a largura visivel da planilha: identificacao, perfil, polo e sinal ficam em cards fluidos, o historico ocupa uma faixa propria e edicao/acessos alinham no inicio sem criar altura vazia. `Excluir conta` usa bloco vermelho recolhido, explica a retencao de 2 anos e exige motivo + confirmacao `EXCLUIR` antes da acao destrutiva.
