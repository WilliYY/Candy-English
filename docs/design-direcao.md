...
- hero da home com video local leve `public/brand/home-candy.mp4` em loop, muted/autoplay, palco amplo e dois videos de intro encaixados nos espacos do mockup; os videos ficam centralizados com fundo branco quando o formato nao fecha com a janela do card, e os controles desenhados no mockup recebem hotspots invisiveis de play/stop/volume, com os videos pausados por padrao para evitar custo inicial alto;
...
- O hero da home usa `public/brand/home-candy.mp4` como palco em loop leve e mais largo no viewport; `public/brand/intro-1.mp4` e `public/brand/intro-2.mp4` ficam encaixados nas janelas dos cards do mockup, centralizados em `object-contain` com fundo branco e com hotspots invisiveis sobre os botoes desenhados de play/stop/volume, sem autoplay nem loop. O video local `public/brand/home.mp4` continua no bloco final de contato como apoio visual leve, com o footer entrando por cima do modulo por animacao gooey, sem degradê de transicao antes do rodape.
...
- Catty deve soar como uma study buddy fofa da Candy: encoraja estudar ingles em metas pequenas, responde em ingles quando a pessoa escreve em ingles, evita frases genericas de chatbot e mantem essa voz tanto na IA via OpenAI quanto no fallback local.
- Catty deve parecer mais viva sem dominar a tela: botao com avatar maior, cabecalho contextual, estado de digitacao e atalhos de estudo adaptados a homework, aulas, mensagens, teacher ou admin. A chamada curta antes da primeira abertura deve aparecer apenas onde nao disputar espaco com WhatsApp, login ou botoes criticos.
