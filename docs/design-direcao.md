...
- logo Candy em loop no header da home e nas paginas internas do site (`/sobre`, `/metodologia`, `/contato`), no topo do AVA, na marca principal do login, em manutencao e no footer por `BrandLogo`, usando WebM transparente, poster PNG para `prefers-reduced-motion` e dimensoes estaveis com `object-contain`; nas paginas internas do site, o header usa uma janela arredondada com `overflow-hidden`, altura maior e escala interna menor para a animacao da bala continuar sem escapar nem cortar na barra branca; no topo interno do AVA a logo animada usa janela arredondada mais alta com `overflow-hidden`, escala controlada e hover leve para aparecer inteira sem passar do limite da sidebar; no topo do login do AVA a logo deve ficar estatica para evitar duas animacoes simultaneas na primeira tela;
