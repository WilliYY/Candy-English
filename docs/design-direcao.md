...
- logo Candy em loop no header institucional, no topo do AVA, na marca principal do login, em manutencao e no footer por `BrandLogo`, usando WebM transparente, poster PNG para `prefers-reduced-motion` e dimensoes estaveis com `object-contain`; no header da home a logo usa uma janela arredondada com `overflow-hidden` para a animacao nao escapar da barra branca; no topo interno do AVA a logo animada usa escala menor e janela arredondada com `overflow-hidden` para nao passar do limite da sidebar; no topo do login do AVA a logo deve ficar estatica para evitar duas animacoes simultaneas na primeira tela;
