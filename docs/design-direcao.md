...
- Student sem foto no perfil ve um popup visual diario apos entrar no AVA, usando somente `public/brand/profile-photo-popup.mp4` em loop leve como conteudo clicavel, borda Candy discreta e botao de fechar; clicar no video vai para `/ava/student?task=perfil`. O popup nao aparece dentro da propria tarefa `Meu perfil` e marca o dia como visto por `localStorage` apenas quando aparece.
