...
O Financeiro do aluno apresenta `Mensalidade`, `Produtos` e total consolidado quando a compra esta vinculada a uma mensalidade aberta. Faturas separadas aparecem no bloco Admin `Produtos de alunos e professores`, identificadas por role, com itens, pendencia e acao de confirmar/reabrir. Desde 10/09/2026, a Teacher tambem confirma/cancela o status no Financeiro: doces vinculados acompanham a mensalidade; compras separadas sao agrupadas por comprador/competencia e alteradas atomicamente com o conjunto esperado de IDs/versoes. Ve itens/quantidades/status de alunos e professores ativos, mas valores somente da propria conta. Toda confirmacao/cancelamento registra autor/data em `FinancialLog` e aviso interno ao Admin. Cancelar pagamento nao estorna a venda nem repoe estoque. O perfil da Teacher mostra um indicador clicavel enquanto existir valor pessoal pendente. Detalhes em `13-financeiro.md`.
