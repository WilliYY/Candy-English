...
- Admin e Teacher acessam `/ava/admin?task=catty-memory` e `/ava/teacher?task=catty-memory` para revisar memorias pessoais; Admin ve tudo e Teacher ve a propria memoria e alunos vinculados. Student acessa `/ava/student?task=catty-memory` apenas como tela informativa `Catty aprendendo`, sem receber a lista tecnica de memorias.
