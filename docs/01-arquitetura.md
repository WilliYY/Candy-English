...
- Admin e Teacher acessam `/ava/admin?task=catty-memory` e `/ava/teacher?task=catty-memory` para revisar memorias pessoais; Admin ve tudo, Teacher ve a propria memoria e alunos vinculados, e Student pode ver a propria memoria em `/ava/student?task=catty-memory` e pedir correcao.
