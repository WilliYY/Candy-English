...
import { reviewMobileTeacherSubmission } from "@/lib/mobile-teacher-submissions";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (session.user.role !== "TEACHER") {
      return mobileError(
        "ROLE_FORBIDDEN",
        "Use uma conta de teacher para corrigir esta entrega.",
        403,
        requestId,
      );
    }
    ...
    console.error("[mobile-teacher-submission:review]", { error, requestId });
    return mobileError(
      "SUBMISSION_NOT_REVIEWED",
      "Nao foi possivel enviar o feedback agora.",
      503,
      requestId,
    );
  }
