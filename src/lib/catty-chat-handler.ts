...
import { pickCattyScenarioFallbackReply } from "@/lib/catty-scenarios";
import {
...
export type AuthenticatedCattyUser = {
  email?: string | null;
  id: string;
  name?: string | null;
  role: Role;
...
const CATTY_SYSTEM_PROMPT = [
  ...
  "Quando a intencao tecnica for `advanced_word_meaning`, explique uma palavra ou expressao curta em ingles simples no formato `[Word or phrase] means... Example: ... Your turn: ...`; nao devolva somente traducao em portugues e use no maximo um sentido extra. Se o alvo passar de 15 palavras, peca uma palavra ou expressao menor.",
  ...
  "Quando o aluno pedir uma pergunta, faca a pergunta diretamente. Use o tema pedido; se nao houver tema, escolha uma pergunta curta de pratica e coloque a traducao logo depois.",
  ...
  "Quando o aluno mandar uma frase curta em ingles, continue o mesmo assunto: elogie ou faca microcorrecao, use historico/memoria se combinar e faca uma unica pergunta relacionada com traducao.",
  "Quando uma correcao local aparecer no prompt, nao peca a frase de novo: use `Better: ...`, `English tip: ...`, `Em portugues: ...` e uma pergunta final em ingles com traducao.",
  ...
  "Se o aluno pedir correcao de ingles, mostre uma versao melhor e explique em ingles e portugues, curto e sem lista longa.",
...
function isUnsafeForResponsePlan(reply: string, plan: CattyResponsePlan) {
  ...
  if (plan.intent === "advanced_word_meaning") {
    if (plan.advancedWordMeaning?.tooLong) {
      return !(
        normalized.includes("shorter") ||
        normalized.includes("up to 15") ||
    ...
    const hasPracticeInvitation =
      normalized.includes("your turn:") || normalized.includes("can you");

    return !hasEnglishDefinition || !hasExample || !hasPracticeInvitation;
  }
...
export async function handleCattyHistoryRequest(
  request: Request,
  user: AuthenticatedCattyUser | null,
) {
  if (!user) {
...
export async function handleCattyChatRequest(
  request: Request,
  user: AuthenticatedCattyUser | null,
) {
  if (!user) {
  ...
  const fallbackReply = applyCattyUserMemoryToFallbackReply({
    ...
    reply:
      ...
      (scenarioFallbackReply ? sanitizeCattyReply(scenarioFallbackReply) : null) ??
