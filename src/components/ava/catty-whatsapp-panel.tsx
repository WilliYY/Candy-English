...
export function CattyWhatsappPanel(props: WhatsappPanelProps) {
  ...
  return <main className="mx-auto w-full max-w-7xl space-y-6 p-4 pb-32 sm:p-6 sm:pb-32">
    <header className="flex flex-wrap items-start justify-between gap-4">
      ...
      <span className={`rounded-full border px-4 py-2 text-sm font-semibold ${props.paused ? "bg-amber-50 text-amber-950" : "bg-emerald-50 text-emerald-950"}`}>{props.paused ? "Envios individuais pausados" : "Envios individuais ativos"}</span>
    </header>
    ...
    <section className={box} aria-labelledby="wa-connection"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 id="wa-connection" className="text-lg font-semibold">1. Conecte o número</h2><p className="mt-1 text-sm text-muted-foreground">WhatsApp: {connection} · Processador: {props.workerOnline ? "online" : "aguardando serviço"}</p></div>
      <div className="flex flex-wrap gap-2"><Button disabled={pending || !props.configured} variant="outline" onClick={() => run("state")}><RefreshCw />Consultar conexão</Button><Button disabled={pending || !props.configured} onClick={() => run("connect")}><QrCode />Gerar QR Code</Button>
        <Button disabled={pending || !props.configured} variant={props.paused ? "outline" : "destructive"} onClick={() => { if (props.paused && !window.confirm("Ativar respostas e envios somente para os contatos autorizados?")) return; void run(props.paused ? "resume" : "pause"); }}>{props.paused ? <Play /> : <Pause />}{props.paused ? "Ativar individuais" : "Pausar individuais"}</Button></div></div>
      ...
      <p className="mt-4 flex items-start gap-2 text-xs leading-relaxed text-muted-foreground"><ShieldCheck className="size-4 shrink-0" />O pareamento não ativa respostas. Esta pausa cancela a fila individual; uma mensagem já em transmissão pode terminar.</p>
      <p className="mt-2 text-sm">O bom-dia no grupo tem controle separado em <Link href="/ava/rotina" className="font-semibold underline">Rotina → Pausar bom-dia</Link>.</p>
    </section>
