...
- Catty como gatinha mascote-professora publica com baloes aleatorios no site e assistente interna do AVA para usuarios logados, com personalidade Candy oficial, primeiro nome seguro em respostas quando fizer sentido, emojis controlados, artefatos meme por interesse do aluno, feedback discreto nas respostas, Learning Center com fila de treino, ate 3 memorias globais aprovadas por Admin no contexto, memoria pessoal ativa e ranqueada por usuario para gostos/preferencias/dificuldades leves, campo minimizado `Contexto Catty` na criacao/aceite de alunos para gravar memoria inicial segura, gestao de memorias em Admin/Teacher/Student, auto-sugestoes pendentes quando houver lacuna de resposta, Gemini padrao, OpenAI apenas quando a mensagem chama Catty pelo nome, fallback local autorizado, historico persistente por usuario/contexto com prompt curto e limpeza manual quando ficar pesado, atalhos de estudo, baloes personalizados, incentivo e pratica simples em ingles;
...
Nao existe ainda: pagamento online, minijogos executaveis de vocabulario/listening/speaking, tela completa de badges/temporadas, tela completa para revisar memorias pessoais da Catty, upload livre de materiais fora dos fluxos interativos, editor Word embutido, relatorios avancados, dashboard complexo e RAG/base de conhecimento automatica ampla para a Catty.
