...
- campos Listening no homework/aula interativa, com leitura OpenAI da frase dentro da area desenhada pela teacher, botao de volume posicionado no fim da area, audio OpenAI text-to-speech em ingles e alternancia normal/devagar a cada clique;
...
- `OPENAI_API_KEY`: chave opcional usada pela Catty somente quando a mensagem chama Catty pelo nome, pelo OCR opcional de homework, pela leitura automatica da frase dos campos Listening e pelo audio OpenAI dos campos Listening; sem chave, Catty cai para Gemini/fallback e Listening deve ser preenchido manualmente.
- `OPENAI_CATTY_MODEL`: modelo OpenAI usado pela Catty no modo acionado por chamada nominal, exemplo `gpt-5.4-nano`.
- `OPENAI_HOMEWORK_OCR_MODEL`: modelo para OCR opcional de homework e para ler a frase dentro do box Listening, exemplo `gpt-4.1-mini`.
