...
- botao `Quero ser aluno Candy` no login do AVA abrindo WhatsApp em nova aba com mensagem pronta; pre-cadastros ficam no fluxo protegido de Secretaria/Admin/Teacher, guardam unidade, teacher responsavel, agenda pretendida e combinado de pagamento, nao sao criados automaticamente por visitante e podem virar aluno real por transaction no botao `Tornar aluno`, abrindo um painel de conversao em etapas com dados do aluno, login revisado, senha inicial visivel/editavel, financeiro obrigatorio, agenda completa ou pendente confirmada e confirmacao antes de criar o acesso;
