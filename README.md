...
- botao `Quero ser aluno Candy` no login do AVA abrindo WhatsApp em nova aba com mensagem pronta; pre-cadastros ficam no fluxo protegido de Secretaria/Admin/Teacher, guardam unidade, teacher responsavel, agenda pretendida e combinado de pagamento, nao sao criados automaticamente por visitante e podem virar aluno real por transaction no botao `Tornar aluno`;
