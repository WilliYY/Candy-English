...
- escolha pos-login limpa para `ADMIN` e `TEACHER`, separando `AVA`, `Secretaria` e `Financeiro` antes de carregar a sidebar de trabalho; o Financeiro da Teacher mostra apenas situacao mensal dos alunos vinculados, sem valores; `STUDENT` entra direto no AVA Student;
- admin com cadastro unico de alunos pela Secretaria, historico de registros antigos, redefinicao de senha, status ativo/inativo, vinculo aluno-teacher, contratos, manutencao, financeiro, agenda, cofre de APIs/senhas, card Admin XP persistente e modulo Candy XP para criar historias/atividades com PDF do Canva e areas interativas sobre o arquivo;
- base de usuarios em planilha/cartoes sem cortar nome ou contato e exclusao controlada: acesso encerrado imediatamente, retencao tecnica por 2 anos e anonimizacao irreversivel dos dados pessoais, sem apagar historicos financeiros, agenda, contratos, vendas ou ponto;
- API mobile ADMIN para listar, buscar, filtrar, paginar, detalhar, criar, editar, ativar/desativar usuarios e redefinir senhas com revogacao imediata das sessoes, protegida pela role autenticada, confirmacao explicita, concorrencia segura e sem expor senha, versao de sessao ou caminhos internos;
...
- API mobile ADMIN para consultar indicadores, ranking privado e atividades Candy XP, editar metadados/liberacoes e corrigir entregas com feedback, versao otimista, idempotencia e concessao transacional de XP, sem expor dados pessoais, caminhos privados ou gabarito fora do detalhe ADMIN;
- teacher com cadastro unico de alunos vinculados ao proprio AVA, acompanhamento mensal sem valores desses alunos e acesso aos registros antigos proprios/atribuidos da Secretaria, aulas interativas por upload do Canva, materiais, vocabulario, homework interativo, feedback, area de aula ao vivo em manutencao temporaria, contratos, mensagens e card Teacher XP persistente;
...
- `docs/21-mobile-admin-candy-xp.md`: atividades, liberacoes, correcoes, indicadores e XP idempotente no app ADMIN.
...
npm run verify:candy-xp-concurrency
npm run verify:candy-xp-submission-concurrency
